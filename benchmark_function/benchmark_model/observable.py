...
from .ode import DifferentialEquation, initial_values
...
def solve_ode(
    ...
    *,
    method: Union[str, OdeSolver] = "BDF",
    ...
    """
    if method not in (available_methods := ["RK23", "RK45", "DOP853", "Radau", "BDF", "LSODA"]) and not (
        inspect.isclass(method) and issubclass(method, OdeSolver)
    ):
        raise ValueError("`method` must be one of {} or OdeSolver class.".format(available_methods))
    if options is None:
        options = {}
...
class Observable(DifferentialEquation):
    ...
    t = range(5401)
    conditions = ["EGF", "HRG"]
    simulations = np.empty((len(obs_names), len(conditions), len(t)))
    normalization: dict = {}
    for observable in obs_names:
        normalization[observable] = {"timepoint": None, "condition": []}
    experiments = [None] * len(obs_names)
    error_bars = [None] * len(obs_names)
    ...
    def simulate(self, x):
        y0 = initial_values()
        # Simulate the model with no ligand until it reaches steady state.
        x[C.Ligand] = x[C.no_ligand]
        sol = solve_ode(self.rhs, y0, range(24 * 3600 + 1), tuple(x))
        if sol is None:
            return False
        else:
            y0 = sol.y[:, -1]
        # add ligand
        for i, condition in enumerate(["EGF", "HRG"]):
            if condition == "EGF":
                x[C.Ligand] = x[C.EGF]
            elif condition == "HRG":
                x[C.Ligand] = x[C.HRG]

            sol = solve_ode(self.rhs, y0, self.t, tuple(x))

            if sol is None:
                return False
            else:
                self.simulations[self.obs_names.index("Phosphorylated_MEKc"), i] = sol.y[V.ppMEKc]
                self.simulations[self.obs_names.index("Phosphorylated_ERKc"), i] = sol.y[V.pERKc] + sol.y[V.ppERKc]
                self.simulations[self.obs_names.index("Phosphorylated_RSKw"), i] = sol.y[V.pRSKc] + sol.y[V.pRSKn] * (x[C.Vn] / x[C.Vc])
                self.simulations[self.obs_names.index("Phosphorylated_CREBw"), i] = sol.y[V.pCREBn] * (x[C.Vn] / x[C.Vc])
                ...
                self.simulations[self.obs_names.index("cFos_Protein"), i] = (
                    (sol.y[V.pcFOSn] + sol.y[V.cFOSn]) * (x[C.Vn] / x[C.Vc]) + sol.y[V.cFOSc] + sol.y[V.pcFOSc]
                )
                self.simulations[self.obs_names.index("Phosphorylated_cFos"), i] = sol.y[V.pcFOSn] * (x[C.Vn] / x[C.Vc]) + sol.y[V.pcFOSc]
